...
# 全局IDP客户端实例（在主线程中初始化）
_global_idp_client = None
_client_credentials_available = False
_client_init_lock = threading.Lock()


def _init_global_idp_client():
    """在主线程中初始化全局IDP客户端"""
    global _global_idp_client, _client_credentials_available
    
    with _client_init_lock:
        if _global_idp_client is not None:
            return
            
        try:
            # 从环境变量或Flask配置获取阿里云凭证
            access_key_id = None
            access_key_secret = None
            
            if current_app:
                access_key_id = current_app.config.get('ALIBABA_ACCESS_KEY_ID')
                access_key_secret = current_app.config.get('ALIBABA_ACCESS_KEY_SECRET')
            ...
            if not access_key_id or not access_key_secret:
                logger.warning("阿里云API凭证未配置，IDP服务将不可用")
                _global_idp_client = None
                _client_credentials_available = False
                return
            ...
            _global_idp_client = docmind_api20220711Client(config)
            _client_credentials_available = True
            logger.info(f"全局IDP服务客户端初始化成功，使用Access Key ID: {access_key_id[:8]}...")
        except Exception as e:
            logger.error(f"全局IDP服务客户端初始化失败: {str(e)}")
            _global_idp_client = None
            _client_credentials_available = False


def init_idp_service():
    """应用启动时调用，在主线程中预初始化IDP服务"""
    _init_global_idp_client()
...
class IDPService:
    ...
    def __init__(self):
        """初始化IDP服务客户端（使用全局客户端实例）"""
        # 确保全局客户端已初始化
        _init_global_idp_client()
        
        # 使用全局客户端实例
        self.client = _global_idp_client
        self.credentials_available = _client_credentials_available
